...
from src.joint_search import run_joint_backtest
...
def main():
    ...
    if args.export_only:
        ...
        with cache.open('rb') as f:forecasts,streams,daily,selections,risk_history,checks,extra=pickle.load(f)
    else:
        forecasts=generate_causal_forecasts(data.dates,data.load,data.pv)
        streams,daily,selections,risk_history,extra=run_joint_backtest(data,forecasts)
        checks={'physical':validate_physics(data,streams),
            'causality':validate_causality(data,forecasts,streams,risk_history)}
        from src.validation import validate_joint_selection
        checks['joint_selection']=validate_joint_selection(data,streams,daily,selections,risk_history,extra)
        with cache.open('wb') as f:pickle.dump((forecasts,streams,daily,selections,risk_history,checks,extra),f)
    ...
    export_results(BASE,data,forecasts,streams,daily,selections,risk_history,checks,make_artifacts=not args.compute_only,extra=extra)
