...
const template = path.join(project, "附件", "附件5", "result2.xlsx");
...
if (mode === "write") {
  ...
  await render("计划购电量","A1:H8","result2_plan");
  await render("充放电量","A1:F14","result2_battery");
  await render("紧急购电量",`A1:C${Math.min(eRows.length,18)}`,"result2_emergency");
